;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
         ...
         ; wydobywanie pmid z artykulu, formatowanie ich pod Pandoc-a ;
                             ; pisane na kolanie ;
           ; ale potem popoprawiane wiec jest raczej przyzwoicie/dobrze ;
...
;; wrapper, konkatenacja stringow ze spacja
(defun s+ (sep &rest stringi)
  (format nil (concatenate 'string "~{~a~^" sep "~}")
          stringi))

(defun my/file->string (filename) ; wczytuje plik jako 1 dlugi string
  (my/concat
...
;; lista tagow do wydobycia z pliku pubmed-23209333.txt
(defparameter *lista-tagow* '("AU" "TI" ((if (eql *skrocona-czy-pelna-nazwa-journala 'pelna)
                                             "JT"
                                             "TA")
...
(setf *manuskrypt* ; wczytuje jako 1 dlugi string
      (my/file->string *manuskrypt-n-pliku*))
...
(loop for pmid in *pmid_unique* and nr-id from 0
   ...
   ;; zapisz plik na dysk
     (my/str->file *str_www* (s+ "" "./pliki_medline/"
					  "pmid_" pmid "_medline"))
...
(setf *tekst-skrypt-med-do-bib-sh*
      (my/file->string "./.skrypt_med_do_bib.sh"))
...
(defun pubmed->authors (tekst)
  body)


;; \A - kiedy linia zaczyna sie od A
;; ^A - kiedy string zaczyna sie od A
(cl-ppcre:all-matches-as-strings "\AU.*" *x*)
